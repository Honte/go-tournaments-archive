...
export function Goban({ className, sgf }) {
  ...
  const linesV = Array.from({ length: board.width })
    .map((_, i) => ({ x1: stepV * (i + 1), y1: stepH, x2: stepV * (i + 1), y2: SVG_BOARD.height - stepH }));
  const linesH = Array.from({ length: board.height })
    .map((_, i) => ({ y1: stepH * (i + 1), x1: stepV, y2: stepH * (i + 1), x2: SVG_BOARD.width - stepV }));
  ...
  return (
    ...
    >
      ...
      {lines.map((line, index) => <line key={index} {...line} stroke="black" strokeWidth="2"/>)}
      {stones.map(([x, y, color], index) => <image
        key={index} x={x} y={y}
        href={color === -1 ? SVG_WHITE.src : SVG_BLACK.src}
        width={stepV} height={stepH}
      />)}
    </svg>
  );
}
