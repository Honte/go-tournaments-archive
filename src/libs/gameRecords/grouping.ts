...
import type { GameBrowserGroupResult, GameBrowserState, GameGroup } from './schema';
...
function getGroupDetails(
  ...
  switch (group) {
    case 'opponent-player':
      return { key: match.opponent.id, label: options.playerMeta.get(match.opponent.id)?.label ?? match.opponent.name };
