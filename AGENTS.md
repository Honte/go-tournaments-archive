...
`npm run build` runs `prebuild` first. `prebuild` generates `public/data` and `public/sgf` assets from the selected
event when `EVENT` is set, or from all events with event-prefixed output when `EVENT` is not set. For local validation,
prefer `npm run build:<event>` or set `EVENT` explicitly before calling raw `npm run build`.
...
Production SGF assets are generated by `tools/assets/` into `public/sgf`. Development SGF responses are handled by
`src/app/sgf/[...path]/route.dev.ts`. Cleaned SGFs, raw SGFs, preview formats, and ZIPs are exposed depending on event
config.
