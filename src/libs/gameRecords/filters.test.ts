...
describe('game filtering', () => {
  ...
  it('distinguishes global, player-relative, and country-relative winner values', () => {
    const blackWins = buildGameRecordsModel(games, state({ winner: 'black' }));
    const whiteWins = buildGameRecordsModel(games, state({ winner: 'white' }));
    const aliceWins = buildGameRecordsModel(games, state({ player: 'a', winner: 'player' }));
    const opponentsBeatBob = buildGameRecordsModel(games, state({ player: 'b', winner: 'player-opponent' }));
    const polishPlayersWin = buildGameRecordsModel(games, state({ country: 'PL', winner: 'country' }));
    const polishOpponentsWin = buildGameRecordsModel(games, state({ country: 'PL', winner: 'country-opponent' }));
    const invalidFocalWinner = buildGameRecordsModel(games, state({ winner: 'player' }));
  ...
  it('filters Jigo games, includes them in winner facets, and reports their availability from all games', () => {
    ...
    const model = buildGameRecordsModel([...games, drawGame], state({ winner: 'jigo' }));
    ...
    assert.equal(buildGameRecordsModel(games, state()).hasJigo, false);
  });

  it('matches non-contiguous tournament-year selections exactly', () => {
    const model = buildGameRecordsModel(games, state({ years: [2020, 2023] }));
