...
export async function Ladder({ stage, players, games, locale }) {
  ...
  return (
    <div className="my-4">
      ...
      <LadderResults stage={stage} players={players} locale={locale} games={games}/>
      ...
      {stage.playoffs?.length ? <div className="my-5">
        ...
        {stage.playoffs.map((game) => <Game key={game} game={games[game]} t={t} players={players}/>)}
      </div> : ''}
    </div>
  );
}
